    ...
    <div className="space-y-4 md:space-y-6 overflow-x-hidden">
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="overview" className="space-y-4 md:space-y-6 mt-4 md:mt-6">
          ...
          {/* Mobile Quick Stats + Filter Buttons - 6 cards total */}
          <div className="md:hidden">
            <div className="grid grid-cols-3 gap-2 mb-4">
              {/* Quick Stats (4 cards) */}
              {quickStats.map((stat, index) => {
                const isActive = false; // Stats don't have active state
                return (
                  <Button
                    key={`stat-${index}`}
                    variant="outline"
                    className={`h-16 p-2 flex flex-col items-center justify-center transition-all duration-300 ease-in-out ${
                      isActive ? 'px-3 gap-1' : 'gap-0.5'
                    }`}
                  >
                    <stat.icon className={`h-4 w-4 flex-shrink-0 ${stat.color}`} />
                    <span className="text-xs font-medium">{stat.value}</span>
                    <span className="text-[10px] text-muted-foreground leading-tight text-center">
                      {stat.label.split(' ').map((word, i) => i === 0 ? word : word.toLowerCase()).join(' ')}
                    </span>
                  </Button>
                );
              })}
              
              {/* Filter Buttons (2 more cards) */}
              {[
                ...
                <Button
                  ...
                  className={`h-16 p-2 flex flex-col items-center justify-center transition-all duration-300 ease-in-out ${
                    activeTab === key ? 'px-3 gap-1' : 'gap-0.5'
                  }`}
                >
                  <Icon className="h-4 w-4 flex-shrink-0" />
                  {activeTab === key ? (
                    <span className="text-xs capitalize animate-in fade-in slide-in-from-bottom-2 duration-300">
                      {key}
                    </span>
                  ) : (
            ...
            {/* Remaining filter buttons below */}
