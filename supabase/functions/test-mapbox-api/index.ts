...
serve(async (req) => {
  ...
  try {
    ...
    const response = await fetch(`https://api.mapbox.com/styles/v1/mapbox/light-v11?access_token=${mapboxToken}`);
    if (!response.ok) {
    ...
    console.error("test-mapbox-api error:", error);
