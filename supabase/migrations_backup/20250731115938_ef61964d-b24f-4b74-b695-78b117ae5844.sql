...
-- Create staff_invitations table with correct structure
CREATE TABLE public.staff_invitations (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  email TEXT NOT NULL,
  role TEXT NOT NULL CHECK (role IN ('super_admin', 'admin', 'moderator', 'manager', 'support', 'staff')),
  invitation_code TEXT NOT NULL UNIQUE DEFAULT encode(gen_random_bytes(32), 'hex'),
  status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'used', 'expired', 'inactive')),
...
CREATE POLICY "Managers can view staff invitations"
ON public.staff_invitations
FOR SELECT
USING (
  EXISTS (
