...
CREATE POLICY "Authenticated users can view public safety settings"
ON public.safety_center_settings
FOR SELECT
USING (
  auth.uid() IS NOT NULL 
  AND setting_type IN ('public', 'default_preference', 'user_setting')
  AND setting_key NOT LIKE '%admin%'
  AND setting_key NOT LIKE '%secret%'
  AND setting_key NOT LIKE '%private%'
);
