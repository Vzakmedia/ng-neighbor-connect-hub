...
-- Conversation tracking table to help organize conversations between users
...
BEGIN
    ...
    IF NOT FOUND THEN
        ...
        INSERT INTO public.conversations (user1_id, user2_id, last_message_at, user1_has_unread, user2_has_unread)
...
CREATE OR REPLACE FUNCTION public.mark_messages_as_read(conversation_id UUID, current_user_id UUID)
RETURNS void AS $$
DECLARE
    conv_record RECORD;
BEGIN
...
-- Create function to check if a user allows messages
CREATE OR REPLACE FUNCTION public.user_allows_messages(user_id UUID)
RETURNS BOOLEAN AS $$
DECLARE
...
-- Create a function to automatically add messaging preferences for new users
...
CREATE TRIGGER on_auth_user_created_messaging
