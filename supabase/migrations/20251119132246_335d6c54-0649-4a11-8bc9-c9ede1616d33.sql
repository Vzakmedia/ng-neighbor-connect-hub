-- Relax geographic targeting for city campaigns so Lagos residents in Lekki see Lagos ads
  ...
  WHERE 
    ...
    AND (
      CASE 
        ...
        WHEN ac.target_geographic_scope = 'city' AND user_state IS NOT NULL THEN 
          -- For city-level campaigns, match any user in the same state
          user_state = ANY(ac.target_states)
        ELSE FALSE
      END
    )
