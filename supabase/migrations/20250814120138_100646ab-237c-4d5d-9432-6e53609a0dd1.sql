-- Priority 3: Seed sample data with valid enum values

-- Insert sample services with valid category enum values
...
(gen_random_uuid(), (SELECT id FROM auth.users LIMIT 1), 'Home Cleaning Service', 'Professional residential cleaning with eco-friendly products. Weekly, bi-weekly, or monthly options available.', 'cleaning', 'hourly', 30, 40, 'Springfield, IL', true, 'approved', 4.9, 43),
(gen_random_uuid(), (SELECT id FROM auth.users LIMIT 1), 'Dog Walking & Pet Sitting', 'Reliable pet care services for busy pet parents. Licensed, insured, and experienced with all breeds.', 'pet_sitting', 'per_visit', 20, 30, 'Springfield, IL', true, 'approved', 4.7, 67),
(gen_random_uuid(), (SELECT id FROM auth.users LIMIT 1), 'Tutoring - Math & Science', 'Expert tutoring for middle and high school students. Improve grades and build confidence in STEM subjects.', 'tutoring', 'hourly', 45, 60, 'Springfield, IL', true, 'approved', 5.0, 28),
(gen_random_uuid(), (SELECT id FROM auth.users LIMIT 1), 'Handyman Services', 'Small repairs, installations, and home improvement projects. No job too small!', 'home_repair', 'hourly', 40, 50, 'Springfield, IL', true, 'pending', null, 0),
(gen_random_uuid(), (SELECT id FROM auth.users LIMIT 1), 'Garden & Landscape Design', 'Transform your outdoor space with professional garden design and maintenance services.', 'gardening', 'project', 200, 500, 'Springfield, IL', true, 'approved', 4.8, 15)
