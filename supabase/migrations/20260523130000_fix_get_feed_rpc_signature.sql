-- Fix get_feed function: restore correct columns, privacy logic, and cast location_scope enum to text to match returned type
...
SELECT cp.id,
    ...
    cp.tags,
    cp.location,
    cp.location_scope::text, -- Cast custom enum location_scope to text to match returned type
...
WHERE -- 1. Optimization: Apply cursor pagination first
    ...
    AND -- 3. Privacy Enforcement (Posts must only be seen if user matches the scope the author declared)
    (
    ...
    AND -- 4. User Preference Filter (Based on Author's Geography)
    (
        -- Neighborhood Filter Level: Only posts authored within user's direct neighborhood
        (
            filter_level = 'neighborhood'
            AND p.neighborhood = user_neighborhood
            AND p.city = user_city
            AND p.state = user_state
        ) -- City Filter Level: Posts from authors in the same city (excluding neighborhood-scoped posts from OTHER neighborhoods)
        OR (
            filter_level = 'city'
            AND p.city = user_city
            AND p.state = user_state
        ) -- State Filter Level: Posts from authors in the same state (excluding scoped posts handled by #3 privacy above)
        OR (
            filter_level = 'state'
            AND p.state = user_state
        ) -- Global / ALL Filter Level
        OR (
